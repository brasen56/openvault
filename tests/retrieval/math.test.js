...
describe('Monotonic extraction-count decay axis', () => {
    const CONSTANTS = { BASE_LAMBDA: 0.05, IMPORTANCE_5_FLOOR: 1.0, reflectionDecayThreshold: 750 };
    const SETTINGS = { vectorSimilarityThreshold: 0.5, alpha: 0.5, combinedBoostWeight: 2.0 };

    it('uses currentExtractionCount - memory.extraction_count when both are present', async () => {
        const { calculateScore } = await import('../../src/retrieval/math.js');
        const memory = { importance: 3, message_ids: [10], extraction_count: 50 };
        // chatLength would normally produce distance = 100 - 10 = 90 (legacy);
        // monotonic axis produces distance = 80 - 50 = 30, so base score is higher.
        const legacy = calculateScore(memory, null, 100, CONSTANTS, SETTINGS, 0, null, null);
        const monotonic = calculateScore(memory, null, 100, CONSTANTS, SETTINGS, 0, null, 80);
        expect(legacy.distance).toBe(90);
        expect(monotonic.distance).toBe(30);
        expect(monotonic.base).toBeGreaterThan(legacy.base);
    });

    it('falls back to legacy axis when memory lacks extraction_count', async () => {
        const { calculateScore } = await import('../../src/retrieval/math.js');
        const memory = { importance: 3, message_ids: [10] };
        // currentExtractionCount supplied but memory not stamped — legacy path runs.
        const result = calculateScore(memory, null, 100, CONSTANTS, SETTINGS, 0, null, 500);
        expect(result.distance).toBe(90);
    });

    it('falls back to legacy axis when currentExtractionCount is null', async () => {
        const { calculateScore } = await import('../../src/retrieval/math.js');
        const memory = { importance: 3, message_ids: [10], extraction_count: 50 };
        const result = calculateScore(memory, null, 100, CONSTANTS, SETTINGS, 0, null, null);
        expect(result.distance).toBe(90);
    });

    it('survives chat compression — distance unchanged when chatLength shrinks', async () => {
        // Simulates Inline Summary collapsing messages: chatLength drops but
        // currentExtractionCount keeps growing (monotonic). Memory distance
        // should reflect the monotonic axis, not the shrunk chat.
        const { calculateScore } = await import('../../src/retrieval/math.js');
        const memory = { importance: 3, message_ids: [50], extraction_count: 200 };

        // Before compression: chatLength=300, currentExtractionCount=300 → distance 100
        const before = calculateScore(memory, null, 300, CONSTANTS, SETTINGS, 0, null, 300);
        // After compression: chatLength=150 (200 msgs collapsed), currentExtractionCount=305
        // Legacy would give distance = 150 - 50 = 100; monotonic gives 305 - 200 = 105.
        const after = calculateScore(memory, null, 150, CONSTANTS, SETTINGS, 0, null, 305);

        expect(before.distance).toBe(100);
        expect(after.distance).toBe(105); // grew by 5 (the new messages since), NOT shrunk
    });

    it('clamps negative distance to 0 if memory was somehow future-stamped', async () => {
        const { calculateScore } = await import('../../src/retrieval/math.js');
        const memory = { importance: 3, extraction_count: 500 };
        const result = calculateScore(memory, null, 0, CONSTANTS, SETTINGS, 0, null, 100);
        expect(result.distance).toBe(0);
    });
});

describe('isCompressedRangeMessage heuristic', () => {
    it('detects messages with extra arrays of message-shaped objects', async () => {
    ...
    it('detects via is_user when name is missing', async () => {
        const { isCompressedRangeMessage } = await import('../../src/extraction/scheduler.js');
        const msg = { extra: { originals: [{ mes: 'hi', is_user: true }] } };
        expect(isCompressedRangeMessage(msg)).toBe(true);
    });

    it('does not flag regular messages', async () => {
        const { isCompressedRangeMessage } = await import('../../src/extraction/scheduler.js');
        expect(isCompressedRangeMessage({ mes: 'hello', extra: {} })).toBe(false);
        expect(isCompressedRangeMessage({ mes: 'hello' })).toBe(false);
        expect(isCompressedRangeMessage({ mes: 'hello', extra: { tags: ['a', 'b'] } })).toBe(false);
    });

    it('does not flag null/undefined input', async () => {
        const { isCompressedRangeMessage } = await import('../../src/extraction/scheduler.js');
        expect(isCompressedRangeMessage(null)).toBe(false);
        expect(isCompressedRangeMessage(undefined)).toBe(false);
    });
});
