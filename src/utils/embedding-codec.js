 ...
 * Returns null if the input is malformed (corrupted chat metadata, partial writes).
 * @param {string} b64 - Base64-encoded Float32Array
 * @returns {Float32Array | null} Decoded embedding vector, or null on failure
 */
function decode(b64) {
    let binary;
    try {
        ...
        console.warn('[OpenVault] Failed to decode embedding (malformed base64), skipping', err);
        return null;
    }
