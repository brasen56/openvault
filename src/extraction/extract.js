...
import { clearAllInjectionSlots, isExtensionEnabled, yieldToMain } from '../utils/st-helpers.js';
...
async function synthesizeCommunities(data, settings, characterName, userName) {
    try {
        ...
        if (error.name !== 'AbortError') {
            recordFailedCall('community', 'Community detection & summarization', error.message || 'Unknown error');
        }
...
async function fetchGraphFromLLM(contextParams, formattedEvents, abortSignal) {
    try {
        ...
        if (error.name === 'AbortError') throw error;
        recordFailedCall('graph', 'Graph extraction', error.message || 'Unknown error');
 ...
 * @param {Object} settings - Extension settings
 * @param {number} extractionCount - Monotonic counter snapshot (post-batch) for decay anchoring
...
async function enrichAndDedupEvents(
    ...
    let events = rawEvents.map((event, index) => ({
        ...
        extraction_count: extractionCount, // Monotonic decay axis — survives chat compression
        created_at: Date.now(),
        batch_id: batchId,
        ...
        is_transient: event.is_transient ?? false,
...
/**
 * Tier 2 contradiction resolution for a freshly extracted batch.
 *
 * For each new event, runs an LLM check against the pre-existing memories (everything
 * except this batch) that share characters and carry opposing sentiment. When
 * `llmContradictionAutoMerge` is enabled, a confirmed contradiction archives the older
 * memory and folds its content into the newer one. Mutations are made in place on
 * `data.memories`; the caller persists them with the final save.
 *
 * A per-batch budget caps how many events reach the (paid) LLM verification step.
 * Called only when `llmContradictionEnabled` is on, and never during backfill.
 *
 * @param {Memory[]} events - Newly extracted memories from this batch
 * @param {object} data - OpenVault chat data (data.memories is mutated in place on merge)
 * @param {object} settings - Extension settings
 * @param {AbortSignal|null} abortSignal
 * @returns {Promise<void>}
 */
async function checkBatchForContradictions(events, data, settings, abortSignal) {
 ...
 * @returns {Promise<{status: string, events_created?: number, messages_processed?: number, reason?: string, attempt?: number, max_attempts?: number, messages_in_batch?: number, event_summaries?: string[]}>}
 */
export async function extractMemories(messageIds = null, targetChatId = null, options = {}) {
    ...
    try {
        ...
        let rawEvents = [];
        try {
            const result = await fetchEventsFromLLM(contextParams, existingMemories, abortSignal);
        ...
        // Pre-compute the post-increment counter so events get the same monotonic stamp
        // that retrieval will compare against after `incrementGraphMessageCount` lands below.
        const extractionCount = (data.graph_message_count || 0) + messages.length;
        ...
        try {
            ...
            if (events.length > 0) {
                ...
                // Stage 5b: LLM contradiction resolution (Tier 2). Retires/merges older
                // memories that the new ones contradict. Off by default; gated by
                // `llmContradictionEnabled`. Mutations persist via the final save below.
                if (settings.llmContradictionEnabled) {
                    await rpmDelay(settings, 'Phase 2 contradiction rate limit');
                    await checkBatchForContradictions(events, data, settings, abortSignal);
                }
            }
...
export async function extractAllMessages(optionsOrCallback) {
    ...
    while (true) {
        ...
        try {
            ...
            // Record for transparency notification
    ...
    // Clear all injection slots and save
