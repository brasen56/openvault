...
// --- Contradiction Verification Schema ---

/**
 * Schema for LLM contradiction verification between two memories.
 * The LLM determines whether two memories about the same characters
 * contradict each other, and optionally suggests a merged summary.
 */
export const ContradictionVerificationSchema = z.object({
    contradicts: z.boolean().describe('Whether the two memories contradict each other'),
    confidence: z
        .number()
        .min(0)
        .max(1)
        .catch(0.5)
        .describe('Confidence level of the contradiction assessment (0.0-1.0)'),
    reason: z.string().min(1).describe('Brief explanation of why they do or do not contradict'),
    newer_is_authoritative: z
        .boolean()
        .catch(true)
        .describe('Whether the newer memory should be treated as authoritative'),
    suggested_merge: z
        .union([z.string().min(1), z.null()])
        .default(null)
        .describe('Merged summary preserving both facts, or null if no merge is appropriate'),
});

/**
 * Get jsonSchema for contradiction verification
 * @returns {Object} ConnectionManager jsonSchema object
 */
export function getContradictionVerificationJsonSchema() {
    return toJsonSchema(ContradictionVerificationSchema, 'ContradictionVerification');
}

/**
 * Parse contradiction verification response
 * @param {string} content - Raw LLM response
 * @returns {Object} Validated contradiction verification
 */
export function parseContradictionVerificationResponse(content) {
    return parseStructuredResponse(content, ContradictionVerificationSchema, (data) =>
