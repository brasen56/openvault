...
/**
 * Detect messages produced by inline-summary extensions (e.g. Inline Summary's
 * `/ils-summarise`) that compress a range of original messages into a single
 * placeholder and store the originals in `extra`.
 *
 * The heuristic: `msg.extra` contains an array of objects that look like chat
 * messages (have a `mes` field and a `name`/`is_user` field). This is the shape
 * Inline Summary uses for its stored originals and is generic enough to cover
 * similar extensions without hard-coding a specific key.
 *
 * Why skip: re-extracting from the summary text would create duplicate memories
 * for content already covered by memories anchored to the original messages,
 * and those new duplicates would have a fresh `extraction_count` and decay
 * slowly while the originals' fingerprints quietly stop resolving.
 *
 * @param {object} msg - Chat message
 * @returns {boolean} True if this looks like a compressed-range summary message
 */
export function isCompressedRangeMessage(msg) {
    if (!msg?.extra || typeof msg.extra !== 'object') return false;
...
export function getUnextractedMessageIds(chat, processedFps, { includeLatest = false } = {}) {
    ...
    for (let i = 0; i < limit; i++) {
        ...
        if (isCompressedRangeMessage(msg)) continue;
