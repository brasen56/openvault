...
1. [★★★★] General Halverson revealed to Captain Yuen that Ironhold garrison has been misled — reinforcements rerouted without their knowledge`,
        thinking: `Step 1: Halverson(PERSON) + Yuen(PERSON) + Ironhold(ORG) + Northern Pass(PLACE) + Supply Line(CONCEPT)
Step 2: All types valid
Step 3: Halverson→Yuen: disclosed+trust; Halverson→Ironhold: withheld reroute; Yuen→Ironhold: now aware; Halverson→N.Pass: strategic focus
Step 4: 5 entities + 4 relationships`,
...
1. [★★★] Petra and Caio lost their quarry after Caio snapped a branch; Petra confronted him with a threat to exclude him from future hunts`,
        thinking: `Step 1: Petra(PERSON) + Caio(PERSON) + Quarry(CREATURE) + Forest(PLACE)
Step 2: Quarry = CREATURE(unidentified); all valid
Step 3: Petra→Caio: command+threat; Caio→Petra: subordinate; Petra→Quarry: 3-day hunt lost; Quarry→Forest: escaped
Step 4: 4 entities + 4 relationships`,
...
1. [★★★★] She initiated first sexual contact with him — unbuckled his belt and gripped his cock; he asked for confirmation and she referenced desire since the conference`,
        thinking: `Step 1: Nadia(PERSON) + Elias(PERSON) + Apartment(PLACE) + Conference(EVENT)
Step 2: Conference = EVENT(prior context); all valid
Step 3: Nadia→Elias: initiated contact+desire; Elias→Nadia: consented+yielded; Nadia→Apartment: encounter site; Nadia→Conference: desire origin
Step 4: 4 entities + 4 relationships`,
...
1. [★★★★] Sable bound Marcus's wrists and arms with jute rope using a structured column tie, confirmed green consent, and ordered him to hold position`,
        thinking: `Step 1: Sable(PERSON) + Marcus(PERSON) + Jute Rope(OBJECT) + Color Protocol(CONCEPT)
Step 2: All types valid
Step 3: Sable→Marcus: dominant+ropework+chin-tilt; Marcus→Sable: submitted+green; Sable→Rope: column tie; Marcus→Color: green consent
Step 4: 4 entities + 4 relationships`,
