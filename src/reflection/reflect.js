...
export function filterDuplicateReflections(
    ...
    for (const ref of newReflections) {
        ...
        const refChar = ref.character?.toLowerCase();
        const sameCharReflections = existingReflections.filter((m) => m.character?.toLowerCase() === refChar);
        let bestMatch = null;
        let bestScore = 0;
...
export async function generateReflections(characterName, allMemories, characterStates, options = {}) {
    ...
    const newReflections = reflections.map(({ insight, importance: llmImportance, evidence_ids }) => {
        ...
        return {
            ...
            sequence: now,
            // Stamp the monotonic decay anchor so reflections fade with the same axis as events.
            // Falls back to undefined when the caller didn't supply it (legacy / unit tests).
            ...(typeof extractionCount === 'number' ? { extraction_count: extractionCount } : {}),
