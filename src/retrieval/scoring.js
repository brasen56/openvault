...
import { DEBUG_TRIMMED_CANDIDATES, IMPORTANCE_5_INJECTION_CAP, OVER_FETCH_MULTIPLIER } from '../constants.js';
...
async function selectRelevantMemoriesSimple(memories, ctx, limit, allHiddenMemories = [], idfCache = null) {
    ...
    const embeddingQuery = buildEmbeddingQuery(userMessagesForEmbedding, queryContext, queryConfig, activeCharacters);
    ...
    return scoreMemoriesDirect(
        ...
        ctx.currentExtractionCount ?? null
    );
}
...
export async function selectRelevantMemories(memories, ctx) {
    ...
    // Keep at most IMPORTANCE_5_INJECTION_CAP importance-5 memories (highest-scoring),
    // then backfill freed slots with the next best non-importance-5 memories.
    if (IMPORTANCE_5_INJECTION_CAP > 0) {
        const imp5InResults = finalResults.filter((m) => (m.importance || 3) === 5);
        if (imp5InResults.length > IMPORTANCE_5_INJECTION_CAP) {
            // Build a set of the top N importance-5 IDs (by score order from scoredResults)
            const imp5IdsByScore = scoredResults
                .filter((r) => (r.memory.importance || 3) === 5 && finalResults.some((m) => m.id === r.memory.id))
                .map((r) => r.memory.id);
            const keepImp5Ids = new Set(imp5IdsByScore.slice(0, IMPORTANCE_5_INJECTION_CAP));

            // Remove excess importance-5 memories and track freed tokens
            let totalTokens = 0;
            for (const m of finalResults) {
                totalTokens += countTokens(m.summary || '');
            }
            const kept = [];
            const removedIds = new Set();
            for (const m of finalResults) {
                if ((m.importance || 3) === 5 && !keepImp5Ids.has(m.id)) {
                    totalTokens -= countTokens(m.summary || '');
                    removedIds.add(m.id);
                } else {
                    kept.push(m);
            ...
            // Backfill with highest-scoring non-importance-5 memories not already selected
            const keptIds = new Set(kept.map((m) => m.id));
            for (const { memory } of scoredResults) {
                if (keptIds.has(memory.id) || removedIds.has(memory.id)) continue;
                if ((memory.importance || 3) === 5) continue; // skip all importance-5 for backfill
                const memTokens = countTokens(memory.summary || '');
                if (totalTokens + memTokens > finalTokens) break;
                kept.push(memory);
                keptIds.add(memory.id);
                totalTokens += memTokens;
            }

            finalResults = kept;
            logDebug(
                `Importance-5 cap: ${imp5InResults.length} -> ${IMPORTANCE_5_INJECTION_CAP} (backfilled to ${finalResults.length} total)`
            );
        }
    }
