 ...
 * @param {number} chatLength - Current chat length (legacy decay axis)
 ...
 * @param {Map<string, number>|null} [chatFingerprintMap] - Fingerprint→index map for legacy fallback
 * @param {number|null} [currentExtractionCount] - Current `data.graph_message_count`; when provided
 *   and the memory carries an `extraction_count` stamp, decay uses the monotonic axis
 *   `currentExtractionCount - memory.extraction_count`. This is immune to chat compression
 *   (e.g., Inline Summary replacing message ranges with a summary).
...
export function calculateScore(
    ...
    /** @type {number|null} */ currentExtractionCount = null
) {
    ...
    // === Forgetfulness Curve ===
    // Prefer the monotonic extraction-count axis when both the memory and the caller
    ...
    if (typeof currentExtractionCount === 'number' && typeof memory.extraction_count === 'number') {
        ...
        distance = Math.max(0, chatLength - maxMessagePosition);
    }
...
export async function scoreMemories(
    ...
    if (queryTokens) {
        ...
        // Also strip multi-word tokens that map to active characters (exact-phrase boost).
        if (characterNames.length > 0) {
            ...
            tokens = tokens.filter((t) => !charStems.has(t) && !charNameSet.has(t.toLowerCase()));
        }
