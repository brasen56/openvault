 ...
 * @param {string[]} [activeCharacters=[]] - Active character names to exclude from prefix
 * @returns {string} Query text for embedding
 */
export function buildEmbeddingQuery(messages, extractedEntities, queryConfig, activeCharacters = []) {
    if (!messages || messages.length === 0) {
        return '';
    }
