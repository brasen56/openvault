...
export function renderMemoryList() {
    ...
    const showArchived = $('#openvault_show_archived').is(':checked');

    let filteredMemories = filterMemories(memories, typeFilter, characterFilter, { showArchived });
    filteredMemories = filterBySearch(filteredMemories, memoryListState.searchQuery);
    const sortOrder = $('#openvault_sort_order').val() || 'date';
    filteredMemories = sortMemories(filteredMemories, sortOrder);
...
/** @type {Set<string>} IDs of memories selected via checkboxes */
...
/**
 * Update the bulk bar visibility and count
 */
function updateBulkBar() {
    const count = selectedMemoryIds.size;
    const $bar = $('#openvault_bulk_bar');
    const $count = $('#openvault_bulk_count');
...
/**
 * Initialize bulk action bar event handlers
 */
function initBulkActionBar() {
    const $list = $(SELECTORS.MEMORY_LIST);

    // Individual checkbox toggle
    $list.on('change', '.openvault-memory-select', function () {
    ...
    $('#openvault_select_all').on('change', function () {
        const checked = this.checked;
        $list.find('.openvault-memory-select').prop('checked', checked);
        $list.find('.openvault-memory-select').each(function () {
            const id = String($(this).data('id'));
            if (checked) {
                selectedMemoryIds.add(id);
            } else {
    ...
    // Bulk archive
    $('#openvault_bulk_archive_btn').on('click', async () => {
    ...
    // Bulk unarchive
    $('#openvault_bulk_unarchive_btn').on('click', async () => {
        const ids = [...selectedMemoryIds];
        if (ids.length === 0) return;
...
export function initBrowser() {
    ...
    // Archived toggle — re-render when checked/unchecked
    $('#openvault_show_archived').on('change', () => {
        memoryListState.page = 0;
        renderMemoryList();
    });

    // Sort order — re-render when changed
    $('#openvault_sort_order').on('change', () => {
        memoryListState.page = 0;
        renderMemoryList();
    });
